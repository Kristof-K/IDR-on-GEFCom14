...
idrOnAll_V1 <- function(X_train, y_train, X_test, print=FALSE) {
  ...
  groups <- c("VAR169" = 1)
  orders <- c("comp" = 1)
  return(invoke_idrOnAll(X_train, y_train, X_test, print, groups, orders))
}
...
# invoke the method
invoke_idrOnAll <- function(X_train, y_train, X_test, print, groups, orders) {
  if (print) {
...
# invoke the method
invoke_idrOnHour <- function(X_train, y_train, X_test, print, groups, orders) {
  if (print) {
    outputForecastingMethod(name_all, var, descr_all)
    return("")
  }
...
idrOnHour <- function(X_train, y_train, X_test, groups, orders) {
  ...
  for (h in hours) {
    indices <- hour(X_train$TIMESTAMP) == h
    trainByHour <- subset(train, indices, select=-TIMESTAMP)
    idr_model[[h]] <- idr(y = trainByHour$POWER, X = trainByHour[-1],
  ...
  # function that makes a right prediction for every row in X_test
  getQuantiles <- function(row) {
    cdf_prediction <- predict(idr_model[[paste0(row$TIMESTAMP)]],
                              data = X_test(notTime))
    quantile_predictions <- qpred(cdf_prediction, quantiles = QUANTILES)
    return(quantile_predictions)
  }
  joinedForecast <- apply(X_test, 1, getQuantiles)
  # apply puts outputs of getQuantiles in columns, but we want that in rows
