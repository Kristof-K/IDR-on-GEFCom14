...
plotsForThesisWind <- function() {
  ...
  for (zone in data$Zones) {
    ...
    data[[zone]]$Train %>% select(-ZONEID, -TIMESTAMP) %>%
      ...
      ylab(target) +
      xlab("ECMF weather forecast") +
      ggtitle(paste("Scatter Plot of Wind Track Colored by Discretized",
                    "Wind Angle")) +
      theme_bw()  +
      theme(legend.position = "bottom", text = element_text(size = 16),
            axis.text = element_text(size = 13)) +
  ...
  top3 <- corr %>% group_by(Zone) %>%
    summarise(Height, Correlation = 32 - rank(Value), .groups = "drop") %>%
    filter(Value <= 3)
  ...
  ggplot(mapping = aes(x = factor(Zone, ordered=TRUE,
    ...
    xlab("Zone") +
    ylab("Height") +
    ggtitle("Spearman Correlation: Wind Speed at Height h - Wind Power") +
    ggsave("WindCorrelationsTask4.png",
             path="plots/ForThesis/", width=18, height=8)
}
